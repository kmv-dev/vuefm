export default () => ({
  url: '',
  radioList: [
    {
      name: 'Радио России',
      site: {
        name: 'radiorus.ru',
        url: 'https://radiorus.ru'
      },
      stream: [
        { name: 'kbps192', url: 'http://icecast.vgtrk.cdnvideo.ru/rrzonam_mp3_192kbps' },
        { name: 'kbps128', url: 'http://icecast.vgtrk.cdnvideo.ru/rrzonam_mp3_128kbps' },
        { name: 'kbps64', url: 'http://icecast.vgtrk.cdnvideo.ru/rrzonam_mp3_64kbps' }
      ],
      diapason: '87.7fm'
    },
    {
      name: 'Радио Культура',
      site: {
        name: 'cultradio.ru',
        url: 'https://cultradio.ru'
      },
      stream: [
        { name: 'kbps192', url: 'http://icecast.vgtrk.cdnvideo.ru/kulturafm_mp3_192kbps' },
        { name: 'kbps128', url: 'http://icecast.vgtrk.ru.ru/rrzonam_mp3_128kbps' },
        { name: 'kbps64', url: 'http://icecast.cdnvideo.cdnvideo.ru/rrzonam_mp3_64kbps' }
      ],
      diapason: '87.7fm'
    },
    {
      name: 'Millennium Radio',
      site: 'ria.ru/radio',
      stream: [
        { name: 'kbps192', url: 'https://myradio24.org/5894' }
      ],
      diapason: '87.7fm'
    }
  ]
})
